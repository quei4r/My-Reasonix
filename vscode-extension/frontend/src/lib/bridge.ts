...
// Resolve the binding at CALL time. In HTTP mode this returns the httpApp proxy;
// in Wails mode it returns the injected window.go.main.App object.
function realApp(): AppBindings | undefined {
  if (isHttpMode()) return httpApp();
...
export function onUpdaterProgress(cb: (p: UpdateProgress) => void): () => void {
  if (isHttpMode()) {
    return subscribeSSE("updater:progress", (p) => cb(p as UpdateProgress));
  }
  if (realApp() && typeof window !== "undefined" && window.runtime) {
...
export function onFilesDropped(cb: (paths: string[]) => void): () => void {
  if (isHttpMode()) {
    // Browser file drops are handled via HTML drag-and-drop, not Wails.
    return () => {};
  }
